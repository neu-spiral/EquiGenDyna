...
from emlp.reps import T as Te, V
from emlp.groups import O2eR3
...
class GatedTransition(nn.Module):
    ...
    def __init__(self, z_dim, transition_dim, S, L):
        ...
        transition_dim = 5*z_dim
        rep_in = uniform_rep(z_dim, G)
        rep_mid = uniform_rep(transition_dim, G)
        rep_out = uniform_rep(z_dim, G) + (z_dim*V**0)(G)
        
        self.linears1 = nn.ModuleList([nn.ModuleList([nn.Sequential(enn.EMLPBlock(rep_in, rep_mid),
                                                                    enn.EMLPBlock(rep_mid, rep_mid))
                                                      for _ in range(L)])
                                       for _ in range(S)])
        
        self.linears2 = nn.ModuleList([nn.Sequential(enn.EMLPBlock(rep_mid, rep_mid),
                                                     enn.Linear(rep_mid, rep_out))
                                       for _ in range(S)])
    ...
    def forward(self, z_t_1):
        ...
        z is L * Batch * z_dim
        """
        L, N, z_dim = z_t_1.shape
        S = len(self.linears1)
        z_params = torch.empty((S, N, 2*z_dim), dtype=torch.float, device = z_t_1.device)
        for s in range(S):
            z_param = 0
            for l in range(L):
                z_param = z_param + self.linears1[s][l](z_t_1[l])
            z_params[s] = self.linears2[s](z_param/L)
              
        # compute the 'proposed mean'
        z_loc = z_params[..., :z_dim]
        # compute the scale used to sample z_t
        z_scale = z_params[..., z_dim:]
...
class StateTransition(nn.Module):
    ...
    def __init__(self, S, factor_dim):
        ...
        if factor_dim:
            #rep_in = rep_mid = uniform_rep(factor_dim, G)
            rep_in = uniform_rep(factor_dim, G)
            rep_mid = uniform_rep(3*factor_dim, G)
            rep_out = (S*V**0)(G)
            
            self.fc_z = nn.Sequential(enn.EMLPBlock(rep_in, rep_mid),
                                      enn.EMLPBlock(rep_mid, rep_mid),
                                      enn.Linear(rep_mid, rep_out))
    ...
    def forward(self, s_t_1, z_t_1):
        ...
        if z_t_1 is None:
            ...
            s_t = self.softmax(self.fc_z(z_t_1))
        return s_t
...
class Emission(nn.Module):
    def __init__(self, factor_dim, D, factorization):
        ...
        rep_in = uniform_rep(factor_dim, G)
        rep_mid = uniform_rep(2*factor_dim, G)
        rep_out = ((D//3)*V)(G)
        if not factorization:
           
            self.fc = nn.Sequential(enn.EMLPBlock(rep_in, rep_mid),
                                    enn.EMLPBlock(rep_mid, rep_mid),
                                    enn.EMLPBlock(rep_mid, rep_mid),
                                    enn.Linear(rep_mid, rep_out))
        else:
            self.fc = enn.Linear(rep_in, rep_out)
...
class EqDDM(nn.Module):
    """
    This PyTorch Module encapsulates the model as well as the
    variational distribution for the EqDDM
    """
    ...
    def fit(self, data, epoch_num = 500):
        ...
        eqddm = self.EqDDM_(self, n_data, lens).to(device)
        
        optim_eqddm = optim.Adam(eqddm.parameters(), lr = self.lr)
        # number of parameters  
        total_params = sum(p.numel() for p in eqddm.parameters())
        learnable_params = sum(p.numel() for p in eqddm.parameters() if p.requires_grad)
        ...
        for i in tqdm(range(epoch_num)):
            ...
            # update EqDDM
                ...
                q_z_mus, q_z_sigs,\
                p_z_mu, p_z_sig\
                = eqddm.forward(mini_batch, mini_batch_idxs)
    
                # set gradients to zero in each iteration
                optim_eqddm.zero_grad()
            
                # computing loss
                idxs_nonnan = ~torch.isnan(mini_batch)
                annealing_factor = 0.001
                
                loss_eqddm = ELBO_Loss(mini_batch[idxs_nonnan],
                                      y_hat[idxs_nonnan],
                                      q_s_0, p_s_0,
            ...
            #torch.save(eqddm.state_dict(), PATH_EqDDM)
        return eqddm
    ...
    def infer(self, data, epoch_num = 500):
        ...
        if self.VI['rnn_dim'] is not None:
            epoch_num = 1
        eqddm = self.fit(data, epoch_num)
        self.grad = True
        return eqddm
         
    class EqDDM_(nn.Module):
        def __init__(self, eqddm, n_data, lens):
            super().__init__()
            ...
            self.softmax = nn.Softmax(dim = -1)
            self.q_s = nn.Parameter(torch.ones(n_data, T, eqddm.S) / eqddm.S, requires_grad=False)
            self.q_s_0 = nn.Parameter(torch.ones(n_data, eqddm.S))
            
            self.q_z_0_mu = nn.Parameter(torch.rand(n_data, max(eqddm.L), eqddm.factor_dim)- 1/2)
            self.q_z_0_sig = nn.Parameter((torch.ones(n_data, max(eqddm.L), eqddm.factor_dim) / 2 * 0.1).log())
            
            self.q_z_mu = nn.Parameter(torch.rand(n_data, T, eqddm.factor_dim)- 1/2)
            self.q_z_sig = nn.Parameter((torch.ones(n_data, T, eqddm.factor_dim) / 2 * 0.1).log())
            if eqddm.VI['rnn_dim'] is not None: 
        ...
        def forward(self, mini_batch, mini_batch_idxs):
            ...
            if self.eqddm.VI['rnn_dim'] is not None:
                
                y_filled = self.eqddm.emission(self.q_z_mu[mini_batch_idxs, :T_b])
                ...
                rnn_output, q_s_t = self.eqddm.lstm_obs(obs.permute(1,0,2))
                
                if self.eqddm.VI['combine']:
                    ...
                    z_prev = z_values.permute(1,0,2)[-np.array(self.eqddm.L)]
                    for i in range(T_b):
                        loc, scale = self.eqddm.combiner(z_prev, rnn_output[i])
            ...
            for lag in self.eqddm.L:
                z_t_1 = torch.cat((z_t_1,
                                   z_values[:, max(self.eqddm.L)-lag:-lag].reshape(1, -1, z_dim)),
                                  dim = 0)

            p_z_mu, p_z_sig = self.eqddm.trans(z_t_1)
            p_z_mu = p_z_mu.view(self.eqddm.S, N, T_b, -1)
            p_z_sig = p_z_sig.view(self.eqddm.S, N, T_b, -1)
            
            # compute q(s_0)
            p_s_0 = self.softmax(self.eqddm.p_s_0)
            q_s_0 = self.softmax(self.q_s_0[mini_batch_idxs])
            # compute q(s_t) = p(s_t|z_t) = p(z_t|z_{t-1},s_t)p(s_t|s_{t-1})
            if not self.eqddm.VI['S']:
            ...
            p_s_t = self.eqddm.strans(q_s[:,:-1],
                                [z_values[:,max(self.eqddm.L)-1:-1]
                                 if self.eqddm.recurrent else None][0])
            z_t_vals = z_values[:, max(self.eqddm.L):] # batch*T_b*z_dim
            
            # p(y|z,F) = Normal(z*F, sigma)

            y_hat = self.eqddm.emission(z_values[:, max(self.eqddm.L):]) # S*N*T*D
            
            # compute q(s_t)
            if not self.eqddm.VI['S']:
                if not self.eqddm.recursive_state:
                    ...
                    # compute q(s_t) = p(s_t|z_t) = p(z_t|z_{t-1},s_t)p(s_t|s_{t-1})
                    p_s_t = torch.Tensor([]).reshape(N, 0, self.eqddm.S)
                    q_s_t = torch.Tensor([]).reshape(N, 0, self.eqddm.S)
                    s_t_1 = q_s_0.clone()
                    for i in range(T_b):
                        # p(s_t|s_{t-1})
                        p_s = self.eqddm.strans(s_t_1,
                                          [z_values[:,i+max(self.eqddm.L)-1]
                                           if self.eqddm.recurrent else None][0]) # batch*S
                        p_s_t = torch.cat((p_s_t, p_s.unsqueeze(1)), dim = 1)
                        z_t_vals = z_values[:, i + max(self.eqddm.L)] # batch*z_dim
                        # compute q(s_t)
            ...
            return y_hat,\
                    ...
                    q_s_t, p_s_t,\
                    q_z_0_mus, q_z_0_sigs,\
                    self.eqddm.z_0_mu, self.eqddm.z_0_sig,\
                    q_z_mus, q_z_sigs,\
                    p_z_mu, p_z_sig
                    
        def report_stats(self, data):
            
            y_recv = self.eqddm.emission(self.q_z_mu).detach().cpu().numpy()*self.eqddm.std + self.eqddm.mean
        ...
        def short_predict(self, s=None):
            ...
            z_t_1 = torch.Tensor([]).reshape(0, N * (T_b-max(self.eqddm.L)), z_dim).to(self.q_z_mu.device)
            for lag in self.eqddm.L:
                z_t_1 = torch.cat((z_t_1,
                                   self.q_z_mu[:, max(self.eqddm.L)-lag:-lag].reshape(1, -1, z_dim)),
                                  dim = 0)
            p_z_mu, p_z_sig = self.eqddm.trans(z_t_1)
            p_z_mu = p_z_mu.view(self.eqddm.S, N, T_b-max(self.eqddm.L), -1)
            p_z_sig = p_z_sig.view(self.eqddm.S, N, T_b-max(self.eqddm.L), -1)
            if s is not None:
                p_z_mu = p_z_mu[[s]]
                p_z_sig = p_z_sig[[s]]
            p_s_t = self.eqddm.strans(self.q_s[:,max(self.eqddm.L)-1:-1],
                                [self.q_z_mu[:,max(self.eqddm.L)-1:-1]
                                 if self.eqddm.recurrent else None][0])
            
            z_val_p = torch.cat(((self.q_z_mu+self.q_z_sig.exp())[:,:max(self.eqddm.L)],
                                 (p_s_t.permute(2,0,1).unsqueeze(-1) * (p_z_mu+p_z_sig.exp())).sum(dim=0)), dim=1)
            z_val_n = torch.cat(((self.q_z_mu-self.q_z_sig.exp())[:,:max(self.eqddm.L)],
                                 (p_s_t.permute(2,0,1).unsqueeze(-1) * (p_z_mu-p_z_sig.exp())).sum(dim=0)), dim=1)
            z_val = torch.cat((self.q_z_mu[:,:max(self.eqddm.L)],
                               (p_s_t.permute(2,0,1).unsqueeze(-1) * p_z_mu).sum(dim=0)), dim=1)
            
            y_pred_n = self.eqddm.emission(z_val_n).detach().cpu().numpy()*self.eqddm.std+self.eqddm.mean 
            y_pred_n = [j[:self.lens[i]] for i, j in enumerate(y_pred_n)]
            y_pred_p = self.eqddm.emission(z_val_p).detach().cpu().numpy()*self.eqddm.std+self.eqddm.mean 
            y_pred_p = [j[:self.lens[i]] for i, j in enumerate(y_pred_p)]
            y_pred = self.eqddm.emission(z_val).detach().cpu().numpy()*self.eqddm.std+self.eqddm.mean 
        ...
        def long_predict(self, steps, s = None):
            ...
            z_values = z_values_p = z_values_n = self.q_z_mu[:,-max(self.eqddm.L):]
            z_t_1 = z_values.permute(1,0,2)[-np.array(self.eqddm.L)]
            z_t_1_s = z_values.permute(1,0,2)[-1]
    
            s_vals = self.q_s[:,-max(self.eqddm.L):]
            s_t_1 = self.q_s[:, -1]
            for i in range(steps):
                p_z_mu, p_z_sig = self.eqddm.trans(z_t_1) # S*N*z_dim
                p_s = self.eqddm.strans(s_t_1,
                                        [z_t_1_s if self.eqddm.recurrent else None][0]) # N * S
                if s is not None:
                    z_val = p_z_mu[s]
            ...
            y_pred_n = self.eqddm.emission(z_values_n[:, max(self.eqddm.L):]).detach().cpu().numpy()*self.eqddm.std+self.eqddm.mean 
            y_pred_n = [j[:self.lens[i]] for i, j in enumerate(y_pred_n)]
            y_pred_p = self.eqddm.emission(z_values_p[:, max(self.eqddm.L):]).detach().cpu().numpy()*self.eqddm.std+self.eqddm.mean 
            y_pred_p = [j[:self.lens[i]] for i, j in enumerate(y_pred_p)]
            y_pred = self.eqddm.emission(z_values[:, max(self.eqddm.L):]).detach().cpu().numpy()*self.eqddm.std+self.eqddm.mean 
            y_pred = [j[:self.lens[i]] for i, j in enumerate(y_pred)]
        ...
        def plot_predict(self, data, steps = None, path = './plots/'):
            ...
            for j in range(0 , len(y_pred), max(len(y_pred)//4, 1)):
                idx_locs = [i for i in range(0, self.eqddm.D, max(self.eqddm.D//5, 1))]
        ...
        def plot_states(self, index = None, k_smooth = None, path = './plots/'):
            ...
            cmap_limited = ListedColormap(colors[:self.eqddm.S])
